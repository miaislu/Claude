...
**修改建议渲染规则（将 `jian-yi-yin-qing` 的 JSON 输出转换为报告第六节）：**
- 每条建议渲染为独立块：问题 → 原文引用（blockquote）→ 替换文本（code block）→ 修改理由 → 双方影响
- 替换文本必须完整可用，不得只写"建议修改此条款"之类的泛泛描述
- 按优先级排序：🔴 必须修改 → 🟡 强烈建议 → 🟢 可选优化
- 单方委托模式：对委托方不利的建议排在前面，用 ⚡ 标记
- 无风险条款时显示 `✅ 未发现需要修改的条款`，不留空节
