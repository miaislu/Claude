...
使用步骤 `10-risk-check` 返回的 `taskId`，调用 `calculatePartyIdentify` **轮询**风险计算结果。
...
**分支逻辑：**
- `partyRiskRes` 非空（存在风险条目，`processStatus=FINISH`）→ 进入步骤 `12-confirm-risk`
- `partyRiskRes` 为空或 null（无风险，`processStatus=FINISH`）→ 直接进入步骤 `13-save-draft`
