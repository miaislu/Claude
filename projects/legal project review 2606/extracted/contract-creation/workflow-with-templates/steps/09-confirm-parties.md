...
gate:
  schema:
    ourParties:
      type: array
      required: true
      desc: "确认后的我方主体列表（OurParty[]，从步骤 08 查询结果中选择）。每项必须包含 partyName、partyIdCard、legalCode 三个字段"
    oppositeParties:
      type: array
      required: true
...
展示步骤 `08-query-our-party` 查询到的我方主体候选列表，以及步骤 `07-confirm-all-info` 中用户填写的对方主体信息，请用户确认或修改。

⛔ **必须等待用户明确确认后，才能调用 `workflow advance`。** 即使我方主体精确匹配唯一一条，也必须将我方主体和对方主体信息一并展示给用户，等待用户回复确认后才能提交。

**交互说明：**
- 我方主体展示规则：
  1. 取步骤 `07-confirm-all-info` 中用户填写的我方主体名称，与 `context.partyList` 中每条记录的 `legalName` 做**精确匹配**（完整名称相等）
  2. **精确匹配到唯一一条** → 展示该条并说明"已自动匹配"，仍需用户确认
  3. **无精确匹配或匹配到多条** → 展示 `context.partyList` 全部结果，让用户手动选择
- 展示对方主体信息，请用户确认是否正确（可修改名称或社会信用代码）
- `contactPersonName`（联系人姓名）固定选填；`contactPhoneNum`/`contactEmail`/`contactAddress` 的展示由 allTemplateFields.subFields 决定，必填性优先由 `formProperty.rules` 判断，其次由 `formFields.subFields.required` 判断（见下方规则）
...
- `region`：注册国家/地区，格式为 `{ code: "国家代码", name: "国家/地区名称" }`（如 `{ code: "CN", name: "中国" }`）。若步骤 `07` 用户已填写则直接沿用；否则应在展示对方主体信息时向用户确认，**未填写则默认为中国**。
...
> ⚠️ 若用户在步骤 `07` 已填写联系人字段，直接沿用，无需再次询问。若未填写且 subFields 包含对应字段，则在确认表中补充展示收集——**展示时只展示字段名，不附加"根据模板要求"等说明**。

> ⚠️ 如果步骤 `07-confirm-all-info` 用户已经填写了联系人字段，必须将其带入到 `oppositeParties` 对象中一起提交，**不得仅写 partyName 并遗漏其他字段**。
...
**对方主体 `entityType` 自动推断规则（继承自 07 推断，此处验证）：**
...
**提示语示例：**
> 以下是查询到的我方主体，请确认选择：
> 1. {{context.partyList[0].legalName}}（统一社会信用代码：...）
> ...
>
> 对方主体信息如下，请确认或修改：
