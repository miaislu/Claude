...
type: interactive
context_mapping:
  - source: "result.get-template-and-form.groupWithFields"
    label: "表单字段分组（包含模板挖空字段和扩展字段）"
    ...
    nested:
      ...
      - key: templateFields
        ...
        nested:
          - key: subFields
            fields: ["fieldCode", "fieldName", "fieldType"]
      - key: allTemplateFields
        fields: ["fieldCode", "fieldName", "fieldType", "fieldIndex"]
        nested:
          - key: subFields
            fields: ["fieldCode"]
  - source: "result.get-template-and-form.formProperty"
...
gate:
  schema:
    ...
    applyCachetReason:
      type: string
      required: false
      desc: "申请原因（stampTypes 包含法人章时必填）"
    ourParties:
      type: array
      required: true
      desc: "我方主体关键词列表（用于步骤 08 查询）"
    oppositeParties:
      type: array
      required: true
    ...
    ext:
      ...
      desc: "扩展字段数组（ExtItem[]）；根据步骤 05 返回的 groupWithFields 中非基础分组的字段收集用户输入，组装为 ExtItem[]；每个 ExtItem 结构为 { fieldGroupCode: string, value: string }，其中 fieldGroupCode 为分组 code（groupCode），value 为该分组内所有字段的 JSON 字符串（key 为 fieldCode，value 为用户填写值）；无扩展字段时可不传"
next_step: query-our-party
---

## 用户一次性填写所有合同信息

⛔ **必须等待用户明确确认后，才能调用 `workflow advance`。** 禁止 Agent 自行推断或预填后跳过用户确认直接提交。

本步骤将 **合同基本信息、用印信息、主体信息、模板挖空字段/扩展字段** 全部合并在一个步骤中收集，用户一次性填写完毕后确认提交。

> ℹ️ **说明**：本流程采用模板创建方式，**发起场景固定为主合同（create）**，无需用户选择。
...
| `stampOrder` | — | 盖章顺序：`{ code: '0', name: '我方先章' }` 或 `{ code: '1', name: '对方先章' }` |
| `stampTypes` | ✅ | 用印类型（多选）：`{ code: '1', name: '合同章' }` / `{ code: '2', name: '公章' }` / `{ code: '3', name: '法人章' }` |
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `ourParties` | ✅ | 我方主体关键词数组（填写主体名称用于查询，可多个） |
| `oppositeParties` | ✅ | 对方主体信息列表（OppositeParty[]） |
...
>
> **【展示判断】** 查看步骤 05（`get-template-and-form`）返回的 `groupWithFields` 中 `partyInfo` 分组下 `allTemplateFields`（优先）或 `templateFields`（fallback）里 `fieldType=17` 条目的 `subFields` 列表：若包含对应 `fieldCode`，则**展示**该字段；否则**完全不展示**（不向用户显示）。  
>
> **【必填判断（双来源，优先级高→低）】**  
...
来自步骤 05（`get-template-and-form`）的 `groupWithFields`，**展示所有非基础分组**（即 `groupCode` 不为 `contractBasicInfo`、`contractStampInfo`、`partyInfo`、`contractAttachment` 的分组）中的字段，并收集用户填写值。

> ⚠️ **每个分组的字段 = `formFields` + `templateFields` 两个列表的合集**（按 fieldCode 去重，以 fieldCode 为唯一键合并）。两个列表可能包含不同的字段，也可能有相同 fieldCode 的字段同时出现在两个列表中——此时该字段同时是表单字段和模板挖空字段，必须展示，必须加 📝 标注。
...
| 3 | 日期 | 日期选择 | `YYYY-MM-DD` 或 `YYYY年MM月DD日` | 毫秒时间戳数字（如 `1780502400000`），由 executor 自动格式化 | `""` （executor 自动转为 `"/"` 传给渲染接口，与前端 `getBEFieldRenderValue` 空值行为一致） |
...
> ⛔ **严禁省略未填写的字段**。无论字段是否必填，只要出现在 `formFields` 或 `templateFields` 中，就**必须**在 `ext.value` 中包含对应的 key，空值用 `""` 表示（表格类型用 `[]`）。executor 会将 `""` 自动转换为 `"/"` 传给渲染接口，与前端 `getBEFieldRenderValue` 的空值处理行为保持一致。
...
> - `executor.ts` 在渲染时会将此对象转换为前端需要的 `{"values":[{"value":"选项名","selected":true},{"value":"选项名2","selected":false},...]}` 格式
...
> - `executor.ts` 在渲染时会将此数组转换为前端需要的 `{"values":[{"value":"选项名","selected":true/false},...]}` 格式
...
用户填写完毕后，Agent 按分组组装 `ExtItem[]` 写入 `gate.confirm-all-info.ext`：
- `fieldGroupCode`：分组的 `groupCode`（来自 `groupWithFields` 中的 `groupCode` 字段，如 `FG250609000001`）
- `value`：**该分组内 `formFields` 和 `templateFields` 中所有字段**的 JSON 字符串，key 为 `fieldCode`，value 为用户填写值（金额带币种类型的值为对象，其余为字符串）

> ⛔ **禁止遗漏 `templateFields` 中的字段**。每个分组的 `templateFields` 是**模板挖空字段**，必须和 `formFields` 一起包含在 `ext.value` 中，否则渲染接口报错。
> ⛔ **禁止添加 `contractBasicInfo` 等基础分组**。`contractBasicInfo`、`contractStampInfo`、`partyInfo`、`contractAttachment` 不得加入 ext。
> ⛔ **禁止使用语义名称作为 fieldCode**。`fieldCode` 必须是后端定义的真实字段编码（格式：字母前缀+数字，如 `CF250609000062`），不得使用 `templateName`、`contractName` 等语义名称。

**ExtItem 组装示例（同一分组含 formFields 字段 CF250609000062 和 templateFields 字段 CF250610000001）：**
```json
[
  {
    "fieldGroupCode": "FG250609000001",
    "value": "{\"CF250609000062\":\"测试字段值\",\"CF250610000001\":\"123\"}"
  }
]
```
...
**⚠️ 展示前静默检查（内部执行，禁止向用户输出检查过程或结论）：**

⛔ **以下检查结果仅用于决定字段是否展示，不得向用户说明"根据模板要求 subFields 包含 XX 字段，因此必填"等解释性文字。**

1. 从 context 中读取 `groupWithFields`，找到 `groupCode === 'partyInfo'` 的分组
2. **【展示检查】** 优先读取该分组的 `allTemplateFields`，若为空则 fallback 到 `templateFields`；在其中找 `fieldType === 17` 条目，读取 `subFields`；若包含 `contactPhoneNum`/`contactEmail`/`contactAddress`，则**展示**该字段；否则不展示（无任何提示）
3. **`contactPersonName`（联系人姓名）固定展示，固定标注为「选填」**，不依赖 subFields 判断
4. 对于展示的联系人字段，**必填性**依次判断：
...
> ⚠️ 「三、主体信息」中联系人字段的**可见性**由 allTemplateFields.subFields 决定，**必填性**优先由 `context.formProperty` 规则引擎决定，其次由 `subFields.required` 兜底，不得混淆两者。每次执行本步骤都必须重新检查，且**不得对用户解释判断过程**。  
> ⚠️ 若步骤 05 返回的 `groupWithFields` 中存在**非基础分组**（groupCode 不为 contractBasicInfo / contractStampInfo / partyInfo / contractAttachment），**必须**在主体信息之后追加对应分组和字段，格式如下：
...
📝 [纯模板挖空字段名称]（选填，模板挖空）：              ← 只在 templateFields 中不在 formFields 中的挖空字段
  填写内容将直接出现在合同正文中，请确认后填写
```

**提示语（紧跟展示内容之后输出）：**

请填写以上信息，填写完毕后请明确回复"确认"。
