...
对步骤 `09-confirm-parties` 确认的**第一个对方主体**调用 `creditPartyIdentify`，发起信用风险识别任务，获取任务 ID（`taskId`）供后续轮询使用。

> 当前仅对 `oppositeParties[0]` 发起任务，多主体场景在 `poll-risk-result` 步骤中扩展。

**分支逻辑：**

- `taskId` **不为空**：表示风险识别任务已创建，进入 `11-poll-risk-result` 步骤轮询结果。
- `taskId` **为空**（接口返回 `data.data = null`）：表示该主体**无风险**，跳过风险校验，直接进入 `13-save-draft` 步骤，不阻断流程。

**存储字段：**

| 字段 | 说明 |
|------|------|
| `taskId` | 风险识别任务 ID，为空表示无风险，传给步骤 `11-poll-risk-result` 使用 |
